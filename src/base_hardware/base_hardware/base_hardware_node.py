...
class BaseHardware(Node):
    ...
    def _publish_state(self, stamp, timed_out: bool):
        payload = {
            ...
            'encoder_maximum_pair_read_duration_s': (
                self.encoder_maximum_pair_duration_s),
    ...
    def _initialize_encoder_feedback(self):
        ...
        self._record_encoder_pair_timing(
            read_finished - read_started, left_first)
    ...
    def _record_encoder_pair_timing(self, duration_s, left_first):
        """Expose the sequential FC03 pair window for dynamic validation."""
        self.encoder_last_pair_duration_s = float(duration_s)
        self.encoder_last_pair_left_first = bool(left_first)
        if (self.encoder_maximum_pair_duration_s is None or
                duration_s > self.encoder_maximum_pair_duration_s):
