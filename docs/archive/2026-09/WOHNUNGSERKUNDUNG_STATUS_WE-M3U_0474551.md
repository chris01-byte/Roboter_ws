...
**Vorhaben WE-1 · Aktualisiert: 2026-09-14 (WE-M3/U)**
...
**WE-M3/U – Der vereinbarte gerätefreie Softwareumfang von Datenzuführung über
automatische Ereignisbildung bis Raum-/Aufgabenverwaltung ist durchgängig
implementiert und prozessübergreifend geprüft.** Ein eigener Prüfer startet den
echten `ExploreNode` in einer isolierten ROS-Domain und führt synthetische
Kartenmanagerstatus-, Rohkarten-, TF-/Scan- sowie Fake-Nav2-Daten durch die
produktive Opt-in-Kette. Im Positivfall wird die Portalaufgabe erst nach dem
realen eingefrorenen Scanmatching und M3/R-Beleg abgeschlossen und die neue
Region betreten. Im Fehlerfall führt fehlendes exaktes TF zu einem Kindzielcancel,
keinem Eintritt und einer weiterhin offenen Portalaufgabe. Beide Szenarien
beobachten null Command-Nachrichten. Der Lauf deckte und schloss zusätzlich die
ROS-Parametertyp-, Lifecycle-Snapshot- und monotone Ereigniszeit-Naht.
...
WE-M3 bleibt formal offen, obwohl sein aktuell vereinbarter gerätefreier
Softwareumfang M3/A bis U umgesetzt ist. Er umfasst Logik, Migrationsvertrag,
...
Regionsfortschritt sind nun im separaten Runtimeprofil gekoppelt. Auch
Portalziel, begrenzte Runtime-Zuführung, eingefrorene LiDAR-Bewegungsreferenz und
atomare Ereignis-/Raum-/Aufgabenfortschreibung sind softwareseitig verbunden.
Der echte Knotenprozess ist nun positiv und abbrechend belegt. Für die formale
Meilensteinabnahme fehlen weiterhin begründete und eingefrorene Zielsystemwerte,
reale Parallel-/SLAM-Last sowie die motorlose Integration mit der unveränderten
vollständigen Sicherheitskette; Softwaretests sind keine Hardwareabnahme.

**Nächste Grenze nach WE-M3/U:** Zunächst die gestapelte Entwicklungsbasis bis
einschließlich PR zu M3/U reviewen und kontrolliert integrieren; kein
automatischer Merge. Danach kann in einem eigenen Auftrag ein gerätefreies,
motorloses Zielsystemprofil mit begründeten Messwerten und paralleler SLAM-Last
geprüft werden. Eine Probefahrt oder WE-M4 ist weder durch diesen Stand
freigegeben noch das vorgezogene nächste Arbeitsziel.
...
| WE-M3/R `feature/we-m3r-portal-traversal-evidence` | Gestapelter ROS-freier Portal-Durchfahrtsbeleg aus frischer Karten-/Posenfolge, vollständigem Chassisauslauf und schlupfresistenter unabhängiger Bewegung; Nav2-Erfolg allein erzeugt kein Ereignis, kein Deployment. |
| WE-M3/S `feature/we-m3s-portal-task-evidence` | Gestapelte scope-gebundene Portalziel- und vollständige Pfadevidenz auf exakter Rohkarte; in diesem Stand nur passive Vorschau und explizite Versandsperre, kein Deployment. |
| WE-M3/T `feature/we-m3t-portal-runtime-evidence` | Gestapelte begrenzte Runtimekopplung von Portalziel, Einzel-Kindziel, exakter Pose, eingefrorener LiDAR-Bewegung und atomarem bestätigtem Traversalereignis; Standardprofil fail-closed, gerätefrei geprüft, kein Deployment. |
| WE-M3/U `feature/we-m3u-process-runtime-evidence` | Gestapelter gerätefreier Vollprozessnachweis des echten Explorer-Opt-in-Profils mit synthetischer DDS-Zuführung, echtem eingefrorenem Scanmatching, positivem Ereignis-/Raum-/Aufgabenpfad und abbrechendem Negativpfad; kein Deployment. |
...
### 2026-09-14 – WE-M3/U: echte Prozessnaht mit synthetischer DDS-Kette

**Entscheidung / Umfang:** Ein neuer gerätefreier Prozessprüfer startet den
produktiven `ExploreNode` als eigenen Prozess und verwendet pro Szenario nur
eindeutige synthetische Topics in einer isolierten ROS-Domain. Er publiziert
bekannte Raster ohne Frontiers, exakte Kartenmanager-Hüllen, dynamisches TF und
720-Strahl-LiDAR-Profile einer festen asymmetrischen Kontur. Ein lokaler
Fake-NavigateToPose-Server besitzt das einzige Kindziel. Zwei reine Subscriber
beobachten die Scan- und Direktfahr-Commandtopics; der Prüfer besitzt keinen
Commandpublisher und öffnet kein Gerät.

Der Prozesslauf deckte drei vorher durch Objekt-Fakes verdeckte
Integrationsfehler auf und schloss sie eng begrenzt: Das optional leere
Scope-Polygon wird im Knoten jetzt explizit als ROS-`DOUBLE_ARRAY` deklariert,
während das typmehrdeutige leere Array aus dem Standard-YAML entfernt wurde.
Der Lifecycle delegiert unveränderliche Portalsnapshots der inneren Sitzung.
Beim Verbuchen eines validierten Traversals liefert der Knoten die vom
Lifecycle verlangte monotone Beobachtungszeit. Das deaktivierte Profil und alle
Fahrsoftwarepfade bleiben ansonsten unverändert.

**Nachgewiesenes Verhalten:** Drei aufeinanderfolgende synthetische
Kartenrevisionen bilden automatisch einen bestätigten Portalbestand, zwei
Regionen und eine offene Portalaufgabe. Im Positivszenario durchläuft ein
reguläres Fake-Nav2-Ziel den konkreten eingefrorenen `LidarReferenceMatcher` mit
exakten TF-Zeitpunkten. Ergebnis sind ein bestätigtes Traversal, ein
Regionseintritt und eine abgeschlossene Portalaufgabe; zusammen mit der vorher
automatisch erledigten Beobachtungsaufgabe stehen zwei Aufgaben auf erledigt.
Im getrennten Fehlerszenario fehlen nach dem Referenzscan absichtlich exakte
TF-Zeitpunkte. Der Monitor meldet
`monitor_failed:exact_time_map_pose_unavailable`, der Kindzielbesitzer storniert
Fake-Nav2 genau einmal, der Auftrag endet erklärt als Systemabbruch, die
Portalaufgabe bleibt offen und der Eintrittszähler bleibt null. In beiden
Szenarien bleibt der Explorerprozess bis zum kontrollierten Testende lebendig
und die Command-Beobachter zählen null Nachrichten.

**WE-M3-Pflichtmatrix für den gerätefreien Stand:**

| Roadmapforderung | Beleg / verbleibende Grenze |
|---|---|
| Hierarchische Auswahl, Alter, Hysterese, Retry und Verhungern | Reine M3/A–C- und I-Verträge sowie deren Lang-/Negativfälle bestanden. |
| Reguläres Nav2-Portal und automatische Durchfahrtserkennung | M3/S–U; U führt die echte Knoten-, DDS-, Scanmatcher- und Fake-Action-Naht positiv aus. |
| Kein falscher Eintritt bei Früh-Erfolg, Schlupf, TF-/Posenfehler, halbem Chassis und Gegenrichtung | M3/R/T-Reinverträge; U belegt zusätzlich den prozessweiten TF-Fehlercancel ohne Eintritt. |
| Kein falscher Vollabschluss und erklärte Teil-/Abbruchzustände | M3/D, O, P und Q; U-Fehlerfall endet als erklärter Systemabbruch, Portalaufgabe bleibt offen. |
| Ein Kindziel, Revisionsinvalidierung und Cancel | M3/J, N und T; U beobachtet je Szenario genau ein Ziel und im Fehlerfall genau einen Cancel. |
| Normale Ziele/Pfade im Scope und getrennte Portalzähler | M3/S/T sowie Scope-/Routenverträge; U nutzt ein explizites synthetisches Polygon und Fortschrittsprüfung. |
| Legacy bei deaktiviertem Profil | Additive M3/E/F-Verträge und unveränderte `false`-/Null-Defaults; kein Deployment. |
| Werte vor Zielsystemabnahme begründet einfrieren | **Offen:** Die U-Werte sind sichtbar synthetisch und keine Chassis-/Sensorvermessung. |
| Motorlose Zielsystemintegration mit unveränderter Gesamtsicherheitskette | **Teilbelegt:** echter lokaler aarch64-Explorerprozess ohne Commandausgabe; vollständiger Bring-up, parallele SLAM-Last und Zielprofil fehlen. |

**Ausgeführte Prüfungen:** Lokaler aarch64-Arbeitsplatz mit ROS Humble, ohne
Geräte, reale Karten, Bags, Command-Publisher oder Bewegung:

| Test-ID | Ergebnis |
|---|---|
| WE-M3U-FOCUS | Lifecycle-, Knoten-, Scope-, Portalmonitor-, Traversal- und Pipelineverträge: **192 passed**. |
| WE-M3U-ADJACENT | Kartenidentität, Explorer, Kartenmanager, Semantikmanager, Mission und Bring-up: **1053 passed**. |
| WE-M3U-COLCON | Frischer temporärer Aufbau von `amadeus_map_identity`, `robot_interfaces` und `explore`; **875 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3U-PROCESS | Frisches Overlay: isolierte Domain 205 positiv **1 Ziel / 1 Eintritt / Portalaufgabe erledigt / 0 Commands**, Fehler **1 Ziel / 1 Cancel / 0 Eintritte / Portalaufgabe offen / 0 Commands**; deaktivierter Standardstart zusätzlich in Domain 206 bestanden. |
| WE-M3U-INSTALL | Explorer- und Lifecycle-Modul sowie Standardkonfiguration aus dem frischen Overlay geladen; Scope-Array im Standardprofil nicht typmehrdeutig gesetzt. |
| WE-M3U-STATIC | `compileall`, `flake8` F/E9 und `git diff --check`: bestanden. |

**Offene Risiken / Integrationsabhängigkeiten:** Der Prozessprüfer simuliert
Geometrie, Kartenmanager, TF, LiDAR und Nav2; er misst weder echte Sensoren noch
Motor- oder Chassiseigenschaften. Die Profilwerte dürfen nicht als Hardwarewerte
übernommen werden. Parallele RTAB-Map-/Nav2-/Collision-Monitor-Last, installierte
Zielprofilkette und reale Kartenrevisionsfrequenz sind nicht geprüft. Insbesondere
kann die gewollte Exaktbindung bei häufigen Kartenänderungen ein laufendes Ziel
abbrechen. Keine bestandene Softwareprüfung behauptet Not-Aus-, Fahr- oder
Hardwareabnahme.

**Nächste Grenze:** Der vereinbarte gerätefreie M3-Softwareumfang endet hier.
Vor einem weiteren funktionalen Schritt müssen die gestapelten PRs reviewt und
kontrolliert integriert werden. Ein späterer motorloser Zielsystemlauf braucht
ein begründetes, eingefrorenes Profil und parallele SLAM-/Sicherheitskette; eine
Fahrt oder WE-M4 erfordert darüber hinaus die Roadmap-Voraussetzungen und eine
neue ausdrückliche Freigabe.

**Rückfallweg:** Den M3/U-Commit beziehungsweise Review-PR zurücknehmen. Als
zusätzliche Laufzeitsperre Monitor-, Navigations- oder gesamtes WE-Profil auf
`false` belassen. Dann bleibt M3/T zwar reviewbar, aber ohne explizites
Double-Array-Scopeprofil und vollständig gesetzte Monitorwerte versandgesperrt.
Der Prüfer hinterlässt nur automatisch gelöschte temporäre Dateien und keinen
Geräte- oder Deploymentzustand.

### 2026-09-14 – WE-M3/T: Portalziel nur mit laufendem Durchfahrtsbeleg

**Entscheidung / Umfang:** Der vorhandene einzelne Nav2-Kindzielbesitzer erhält
einen begrenzten synchronen Fortschrittsbeobachter. Für Portalaufgaben wird er
nur erzeugt, wenn Wohnungserkundungsnavigation, verifizierter Auftragsscope,
eingefrorener LiDAR-Bewegungsmodus und der neue Traversierungsmonitor gemeinsam
explizit aktiviert sind. Sämtliche geometrischen und zeitlichen Profilwerte
müssen positiv beziehungsweise endlich gesetzt sein; die Standardkonfiguration
bleibt mit `false` und Nullwerten geschlossen.

Der Monitor friert vor der Portalquerung einen ausreichend großen frischen
LiDAR-Scan ein, ordnet Folgescans exakt gestempelte Kartenposen zu und ermittelt
die Bewegung gegen diese feste Referenz. Er akzeptiert weder Radencoder allein
noch Quellenwechsel, Replay, zu alte Daten, Sprünge, zu viele Ablehnungen oder
ein Verlassen des vollständigen Scope-Abstands. Die gepaarten Werte werden
ausschließlich über den bestehenden M3/R-Vertrag bewertet. Eine positive
Bewertung erzeugt genau ein typisiertes Traversalereignis und verbucht es atomar
im Schattenzustand; dadurch wechseln aktuelle Region und offene Portalaufgabe
gemeinsam. Bei Monitorfehler wird das aktive Kindziel abgebrochen. Ein reiner
Nav2-Erfolg ohne bestätigtes Ereignis bleibt ein begrenzter Retry und darf weder
Region noch Aufgabe fortschreiben.

**Nachgewiesenes Verhalten:** Ein reines durchgängiges Szenario führt wechselnde
exakt korrelierte Rohkarten automatisch über stabilen Portalbestand,
Portalbestätigung, vorläufigen Regionsgraphen und offene Portalaufgabe bis zu
Scope-Route, Zielabsicht, gepaarter Traversalevidenz, bestätigtem Ereignis,
Regionswechsel und erledigter Aufgabe. Knotentests belegen zusätzlich den
positiven Monitorpfad, den wirkungslosen Action-Erfolg ohne Ereignis, Abbruch bei
Beobachterfehler, eingefrorene Scan-Konvertierung und exakt gestempeltes TF.
Ein lokaler DDS-Smoke verwendete einen Fake-NavigateToPose-Server und den echten
Kindzielbesitzer; sieben synthetische Beobachtungen ergaben ein bestätigtes
Ereignis, während Beobachter für Geschwindigkeitskommandos null Nachrichten
sahen. Dieser Smoke injiziert das Monitorurteil und ersetzt daher nicht den
reinen Test des konkreten Scan-Matchers oder einen vollständigen Knotenstart.

**Ausgeführte Prüfungen:** Lokaler aarch64-Arbeitsplatz mit ROS Humble;
ausschließlich synthetische Raster, Scans, Posen, TF-Ersatzwerte und ein
Fake-Actionserver, keine Geräte, realen Karten, Bags oder Bewegung:

| Test-ID | Ergebnis |
|---|---|
| WE-M3T-FOCUS | Scope-, Portalziel-, Traversal-, Matcher-, Pipeline-, Graph-, Kindlauf- und Explorer-Knotenverträge: **207 passed**. |
| WE-M3T-ADJACENT | Kartenidentität, Explorer, Kartenmanager, Semantikmanager, Mission und Bring-up: **1052 passed**. |
| WE-M3T-COLCON | Frischer temporärer Aufbau von `amadeus_map_identity`, `robot_interfaces` und `explore`; **874 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3T-DDS | Frisches Overlay, isolierte Domain 194: **1** Fake-Nav2-Ziel, **7** Monitorbeobachtungen, **1** Ereignis, **0** Task-Attempts und **0** beobachtete Command-Nachrichten. |
| WE-M3T-INSTALL | Neue Runtime-/Scanmodule und die installierte Konfiguration aus dem frischen Overlay importiert beziehungsweise gefunden. |
| WE-M3T-STATIC | `compileall`, `flake8` F/E9 und `git diff --check`: bestanden. |

**Offene Risiken / Integrationsabhängigkeiten:** Die LiDAR-, Kontur-, Zeit- und
Qualitätsgrenzen sind synthetische Testwerte und keine Chassisvermessung oder
Zielsystemfreigabe. Scanmatching gegen eine eingefrorene Referenz ist
softwareseitig plausibilisiert, aber nicht auf Jetson unter paralleler SLAM-Last
oder bei realer Sichtänderung belegt. Der kleine DDS-Smoke startet nicht den
vollständigen Explorer-Prozess und injiziert das Monitorergebnis. Außerdem kann
die absichtlich exakte Revisionsbindung ein laufendes Portalziel bei jeder
inhaltlich neuen Karte abbrechen; ob die reale Kartenfrequenz ausreichend lange
stabile Fenster liefert, ist offen. Softwaretests sind keine Fahr- oder
Hardwareabnahme.

**Nächster abgegrenzter Schritt WE-M3/U:** Einen vollständigen synthetischen
Prozessprüfer für das echte Explorer-Profil aufbauen. Er speist Kartenstatus,
Rohkarte, Pose/TF, Scan und Fake-Nav2 in einer isolierten Domain, belegt den
positiven Ereignis-/Raum-/Aufgabenpfad sowie mindestens Monitorfehler,
Kindzielabbruch und begrenzte Statusdiagnose. Danach den gerätefreien
M3-Softwareumfang gegen `MEILENSTEINE.md` abgrenzen; Zielsystemwerte und reale
Fahrt bleiben getrennte, freigabepflichtige Arbeiten.

**Rückfallweg:** Den M3/T-Commit beziehungsweise Review-PR zurücknehmen oder den
Monitorparameter auf `false` belassen. Dann bleibt die M3/S-Portalvorschau
sichtbar, ihr Versand aber ausdrücklich gesperrt; Frontier-Navigation und
Legacy-Explorer bleiben auf dem vorherigen Verhalten. Es gab kein Deployment
...
**Entscheidung / Umfang:** Ein neues ROS-freies Modul validiert den Übergang
zwischen den kanonischen Seiten eines bestätigten Portals. Das Profil muss
Chassisüberhänge, Vor-/Nachlauf, laterale Toleranz, maximale Pose-/Yaw-Sprünge,
Geschwindigkeit, Rückschritt, Bewegungsabweichung, Quellalter, Revisionsabstand
und Mindestzahl der Posen explizit liefern. Diese Werte besitzen absichtlich
keine als Hardwaremessung deutbaren Vorgaben.

Reguläres Nav2 und die vorhandene Sonderbrücke durchlaufen denselben Vertrag.
Ein Ausführungserfolg genügt nicht. Alle Posen müssen im Kartenkontext liegen,
zeitlich und revisionsseitig geordnet, entlang der angeforderten Portalseite
ausgerichtet und im begrenzten Korridor sein. Die Vorderkante muss anfangs
vollständig vor der Startseite, die Hinterkante am Ende vollständig hinter der
Gegenseite liegen. Zusätzlich muss eine schlupfresistente unabhängige
Bewegungsmessung im exakt gleichen Zeitfenster mit der lokalisierten Bewegung
übereinstimmen. Reine Radencoder-Evidenz ist ausdrücklich unzureichend.

**Nachgewiesenes Verhalten:** Eine synthetische reguläre Nav2-Querung und die
gleiche Folge im Sonderbrückenmodus erzeugen deterministisch dasselbe typisierte
bestätigte Ereignisformat. Die Gegenrichtung wird separat kanonisch verbucht.
Früher Nav2-Erfolg, zu wenige Posen, halbes Chassis im Durchgang,
Encoder-only/Schlupf, TF-Positionssprung, widersprechende unabhängige Bewegung,
falsche Ausrichtung, laterales Verlassen, Rückschritt, stale Karte/Revision,
fremder Kontext und falsches Messzeitfenster erzeugen kein `TraversalEvent`.

**Ausgeführte Prüfungen:** Lokaler aarch64-Arbeitsplatz mit ROS Humble, aber
ohne ROS-Start; ausschließlich synthetische Zahlenwerte, keine Geräte, realen
Karten, Bags oder Bewegung:

| Test-ID | Ergebnis |
|---|---|
| WE-M3R-FOCUS | Portalbeleg-, Portalgedächtnis-, Graph-, Schatten- und Szenarioverträge: **284 passed**. |
| WE-M3R-ADJACENT | Kartenidentität, Explorer, Kartenmanager, Semantikmanager, Mission und Bring-up: **981 passed**. |
| WE-M3R-COLCON | Frischer temporärer Aufbau von `amadeus_map_identity`, `robot_interfaces` und `explore`; **827 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3R-STATIC | `compileall`, `flake8` F/E9 und `git diff --check`: bestanden. |

**Offene Risiken / Integrationsabhängigkeiten:** Noch kein Runtimepfad sammelt
die typisierten Posen und die unabhängige Bewegung oder bindet das Ereignis an
den Schattenlebenszyklus. Portalaufgaben besitzen weiterhin keinen Zielpunkt.
Die Grenzwerte sind synthetische Prüfwerte und vor einer Zielsystem- oder
Hardwareprüfung zu begründen und einzufrieren. VIO als tatsächlich unabhängige
Quelle ist nicht nachgewiesen; die vorhandene eingefrorene LiDAR-Messidee muss
bei der späteren Kopplung quellengetrennt bleiben. Softwaretests sind keine
Fahr- oder Hardwareabnahme.

**Nächster abgegrenzter Schritt WE-M3/S:** Portalaufgabe, bestätigte
Portalgeometrie, exakte Rohkarte und geprüften Auftragsscope in einen regulären
Nav2-Zielkandidaten auf der Gegenseite überführen. Danach ausschließlich über
den vorhandenen Einzel-Kindzielbesitzer mit synthetischen Fake-Actions erproben
und die M3/R-Evidenz begrenzt zuführen. Keine Sonderblindfahrt und keine Geräte.

**Rückfallweg:** Den M3/R-Commit beziehungsweise Review-PR zurücknehmen. Da das
neue Modul noch keinen Runtime-Aufrufer besitzt, entfällt nur der reine
Prüfvertrag; M3/Q, Legacy-Explorer, Nav2 und Geräteverhalten bleiben unverändert.
Es gab kein Deployment und keinen Gerätezustand zurückzusetzen.
...
**Entscheidung / Umfang:** Zwischen den erfolgreichen M3/N-Kindlauf und eine
Graphaufgaben-Erledigung eine neue reine Evidenzstufe gesetzt. Der Erfolg selbst
erzeugt nur einen wartenden Prüfauftrag aus exakt gebundenem Zielkandidaten und
M3/L-Disposition. Erst eine inhaltlich neuere Rohkarte desselben Kontexts darf
entscheiden. Sie wird erneut gegen Fingerprint, Stempel, Frame, Zellzahl und
Geometrie validiert.

Eine Frontier gilt nur dann als positiv erledigt, wenn kein Track der aktuellen
Revision im bisherigen Informationsradius liegt, die tatsächlich angefahrene
Zielkoordinate weiterhin bekannte freie Zelle ist und das vollständige
kreisförmige Informationsfenster um den alten Frontieranker innerhalb der Karte
liegt und keine unbekannte Zelle mehr enthält. Ein historischer Track bleibt im
Gedächtnis und ist erlaubt; ein aktueller gleicher oder neu zugeordneter naher
Track sperrt dagegen. Nur `resolved` darf die exakt passende offene
Frontier-Aufgabe über Schatten-Sitzung und Lebenszyklus atomar auf `completed`
setzen. Identische Anwendung ist idempotent.

**Nachgewiesenes Verhalten:** Weiterhin sichtbare und nahe neu identifizierte
Frontiers, unbekannte Zellen, belegte Zielzelle, am Kartenrand abgeschnittenes
Fenster, alte/fremde Karte, nicht erfolgreicher Kindlauf, doppelte oder
zukünftige Tracks sowie Kapazitätsüberschreitung schließen die Erledigung aus.
Positive Evidenz schließt genau die gebundene Aufgaben-ID, nicht eine Aufgabe
nur aufgrund räumlicher Nähe. Der Node verwirft Ergebnisse bei zwischenzeitlich
gewechselter Quelle und veröffentlicht den Prüfzustand begrenzt im vorhandenen
WE-Status.

**Ausgeführte Prüfungen:** Lokaler aarch64-Arbeitsplatz mit ROS Humble, ohne
Gerätezugriff, reale Karte oder Bewegung:

| Test-ID | Ergebnis |
|---|---|
| WE-M3P-FOCUS | Neue Evidenz-, Schatten-, Lebenszyklus- und Node-Vertragstests: **206 passed**. |
| WE-M3P-ADJACENT | Kartenidentität, Explorer, Kartenmanager, Semantikmanager, Mission und Bring-up: **958 passed**. |
| WE-M3P-COLCON | Frischer temporärer Aufbau von `amadeus_map_identity`, `robot_interfaces` und `explore`; Explorer-Pakettest: **768 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3P-STATIC | `compileall`, `flake8` F/E9 und `git diff --check`: bestanden. |

Die Runtimekopplung wurde mit injizierten, synthetischen exakten Quellen und
dem echten Schatten-Lebenszyklus geprüft. Es gab in diesem Schritt keinen neuen
ROS-Start; der M3/O-DDS-Nachweis bleibt der letzte motorlose Action-Smoke. Weder
Softwaretests noch der frühere DDS-Smoke sind eine Jetson- oder Hardwareabnahme.

**Offene Risiken / Integrationsabhängigkeiten:** Das Informationsfenster und die
übrigen Werte sind weiterhin synthetische Startwerte. Der Aufgabenabschluss
setzt den Erkundungszustand der betroffenen Region noch nicht automatisch fort;
dadurch kann der M3/O-Abschlussautomat trotz leerem Aufgabenbestand konservativ
gesperrt bleiben. Portal- und Beobachtungsaufgaben besitzen noch keine
Zielableitung. Auftragsscope und vollständige Nav2-Pfade sind noch nicht durch
ein eingefrorenes Testprofil begrenzt.

**Nächster abgegrenzter Schritt WE-M3/Q:** Graph-/Schattenlogik so ergänzen,
dass eine neue offene Aufgabe die betroffene Region nachvollziehbar auf
`in_progress` setzt und eine positive Erledigung nur bei leerem regionalem
Aufgabenbestand `complete` ergibt. Neue Aufgaben nach vermeintlichem Abschluss,
Portal-/Beobachtungsblocker, Replay sowie Merge/Split testen. Keine Zielpose,
Geräteaktivierung oder Fahrt.

**Rückfallweg:** Den M3/P-Commit beziehungsweise Review-PR zurücknehmen. Dann
bleibt der M3/N-Erfolg wie zuvor nur Fortschritts-/Attemptbeleg und keine
Frontier-Aufgabe wird automatisch erledigt. Alternativ das standardmäßig falsche
WE-Navigations-Opt-in beibehalten. Es gab kein Deployment und keinen
Gerätezustand zurückzusetzen.

### 2026-09-14 – WE-M3/O: Abschlussautomat im WE-Runtimeprofil

**Entscheidung / Umfang:** Den in M3/D definierten Abschlussautomaten an die
einmal je Kartenrevision fortgeschriebene Policy aus M3/I gekoppelt. Das neue
Profil bleibt standardmäßig aus. Selbst bei aktivem Profil ist
`wohnungserkundung_accessible_scope_verified` standardmäßig `false`; erst ein
explizit nachgewiesener Auftragsscope und standardmäßig drei aufeinanderfolgende
frische qualifizierte Revisionen erlauben `complete_accessible`. Jede
Beobachtung ist an den exakten Kartenkontext und die Quellrevision gebunden und
prüft, dass kein Kindziel aktiv ist.

Externe Terminalursachen brauchen bewusst keine künstliche neue Kartenrevision:
Zeit-/Ziellimit erzeugt `partial`, interner Fehler oder ROS-Ende `aborted`,
Nutzerabbruch `canceled`. Der M3/E-Vertrag projiziert diese Zustände in die
unveränderte `ExploreArea`-Action und ergänzt den bestehenden JSON-Status unter
`wohnungserkundung.runtime_result`. Ein Teilstand beendet die Action regulär,
trägt aber weiterhin `success=false`; nur `complete_accessible` setzt den
bestehenden Vollabschlussbeleg.

**Nachgewiesenes Verhalten:** Der reine Automat bewahrt die Zahl bereits
qualifizierter Revisionen bei einem externen Ende, akzeptiert identische
Terminal-Replays idempotent und weist widersprüchliche Wiederholungen zurück.
Voll-, Teil-, System- und Nutzerende erhalten unterschiedliche Actionterminals.
Der Runtimepfad prüft Abschluss vor der Bildung eines weiteren Ziels. Ohne
expliziten Bereichsnachweis kann ein ruhiger Aufgabenbestand keinen
Vollabschluss erzeugen. Das deaktivierte Profil verändert den Legacypfad nicht.
...
| Test-ID | Ergebnis |
|---|---|
| WE-M3O-FOCUS | Abschluss-, Migration-, Node- und Vertragsprüfungen gemeinsam: **163 passed**. |
| WE-M3O-ADJACENT | Kartenidentität, Explorer, Kartenmanager, Semantikmanager, Mission und Bring-up: **946 passed**. |
| WE-M3O-COLCON | Frischer temporärer Aufbau von `amadeus_map_identity`, `robot_interfaces` und `explore`; Explorer-Pakettest: **756 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3O-DDS | Isolierter Domain-Smoke mit echtem Explorer- und Actionserver: Zeitlimit → Actionstatus `SUCCEEDED`, Ergebnis `success=false`, Runtimezustand `partial`; **0 Nav2-Ziele, 0 Nachrichten auf `/cmd_vel` und `/cmd_vel_explore_direct_raw`**. |
| WE-M3O-STATIC | `compileall`, `flake8` F/E9 und `git diff --check`: bestanden. |

Der DDS-Test bestätigt nur die Softwareverdrahtung im Teilstandspfad. Keine
Sensor-, TF-, Controller-, Jetson-, Not-Aus- oder Hardwareabnahme wurde
ausgeführt oder daraus abgeleitet.

**Offene Risiken / Integrationsabhängigkeiten:** Das Bereichsgate ist noch ein
expliziter Softwareeingang und kein freigegebenes räumliches Testprofil. Vor
einem realen Einsatz müssen Scope, normale Nav2-Ziele und vollständige Pfade
gemeinsam begrenzt sowie Werte begründet eingefroren werden. Erfolgreiche
Navigation markiert eine Frontier absichtlich noch nicht als erledigt; ohne
positive nachfolgende Kartenevidenz bliebe die Aufgabe offen. Portal- und
Beobachtungsaufgaben besitzen noch keine Runtime-Zielableitung. Motorlose
Zielsystemlast, Sicherheitskette und Abbruch auf dem maßgeblichen Jetson sind
weiter offen.

**Nächster abgegrenzter Schritt WE-M3/P:** Eine reine, revisionsgebundene
Frontier-Erledigungsevidenz ergänzen und nach einem erfolgreichen Kindziel in
Graph und Policy zuführen. Nur ein neuer exakt korrelierter Rohkartenbestand,
kein aktueller Track und ein vollständig bekannter Ziel-/Informationsbereich
dürfen die Aufgabe erledigen. Replay, bloßes Detektorverschwinden, Filterung,
mehrdeutige Tracks und Kontextwechsel negativ testen. Keine Portalziele,
Geräteaktivierung oder Fahrt.

**Rückfallweg:** Den M3/O-Commit beziehungsweise Review-PR zurücknehmen oder das
standardmäßig falsche Navigations-Opt-in beibehalten. M3/A bis N bleiben separat
reviewbar; die Legacy-Schleife bleibt dann unverändert aktiv. Es gab weder
Deployment noch einen Gerätezustand zurückzusetzen.
...
**Arbeitsbasis und Umfang:** `feature/we-m3m-runtime-goal-preview` baut direkt
auf M3/L `cd29ee7` auf. Bei aktivierter passiver WE-Policy übernimmt der
vorhandene 1-Hz-Schattenpfad den exakt passenden Rohkartensnapshot, aktuellen
Trackbestand und die außerhalb der Sperre ermittelte Roboterpose. Erst nach
passiver sowie zustandsbehafteter Bewertung wird eine Absicht erzeugt, sofern
die aktuelle Bewertung weiterhin frisch, `ready_with_tasks` und für dieselbe
Aufgabe geeignet ist. Kandidaten werden höchstens einmal je Kombination aus
Fingerprint, Quellstempel, Frame, Kartenrevision, Roboterzelle und Absichts-ID
berechnet.

**Diagnose- und Wirkungsgrenze:** Unter `wohnungserkundung.goal_candidate`
stehen bei einem aktuellen Vorschlag IDs, Revision, Frame, vollständige
Quellbindung, Rasterzelle, metrisches X/Y/Yaw, Frontierpunkt, geodätische Länge
und Informationsfläche. Das Feld `navigation_dispatched` ist unveränderlich
`false`. Ohne exakte Eingänge oder bei stale/zurückgehaltener Auswahl enthält
die Diagnose nur einen expliziten Nichtverfügbarkeitsgrund. Ein Fehler der
Zielkandidatenbildung sperrt den Kandidaten, nicht den bestehenden Shadow-
Publisher. Actionserver, Nav2-Client, Legacyausgabe und Twist-Publisher lesen
den Vorschlag nicht.

**Ausgeführte Prüfungen:** Lokaler x86_64-Arbeitsplatz mit ROS Humble und
frischem temporären Overlay; synthetische Karten/TF in isolierter DDS-Domain,
keine Geräte, realen Karten, Bags, Actionziele oder Bewegung:

| Test-ID | Ergebnis |
|---|---|
| WE-M3M-FOCUS | Kindziel-, Migration- und Nodeverträge: **110 passed**; darunter Live-Auswahlbindung, begrenzte Projektion, Berechnung außerhalb der Sperre und Cache-Replay. |
| WE-M3M-EXPLORER | Vollständige Explorer-Suite: **727 passed**. |
| WE-M3M-ADJACENT | Explorer plus vollständige Fingerprint-, Karten-, Semantik-, Mission- und Launch-Suiten: **917 passed**. |
| WE-M3M-COLCON | Frischer Build von `amadeus_map_identity`, `robot_interfaces` und `explore`; **36 + 727 = 763 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3M-ROS | Isolierte lokale DDS-Domain: `ready_with_tasks`, aktueller Kandidat für `task-frontier_000001`, Kartenframe und exakte Revision, `navigation_dispatched=false`, **0 Actions**, **0 Command-Nachrichten**. |
| WE-M3M-STATIC | `compileall`, auf geänderte Zeilen begrenztes `flake8` mit E501/W503-Ausnahmen und `git diff --check`: bestanden. |

**Offene Grenzen:** Der Vorschlag ist weiterhin rein diagnostisch. Der
vorhandene Nav2-Client sendet ihn nicht; Kindzielautomat und Resultatadapter
besitzen noch keinen Runtime-Aufrufer. Portal-/Beobachtungsaufgaben bleiben
ohne spezifische metrische Zielevidenz gesperrt. Abschlussruntime, aktives
Profil, Scope-/Pfadgrenzen, Jetsonlast und Hardwareprüfung fehlen. Softwaretests
und der lokale DDS-Lauf sind keine Fahr- oder Hardwareabnahme.

**Nächster abgegrenzter Schritt WE-M3/N:** Ein eigenes, standardmäßig
deaktiviertes WE-Navigationsprofil ausschließlich innerhalb der vorhandenen
Explorer-Orchestrierung anbinden. Exakt ein aktueller Kandidat darf als
Kindziel gesendet werden; Kartenwechsel muss Cancel vor Neuziel erzwingen und
das Ergebnis muss in den bestehenden reinen Vertrag zurückfließen. Nur
Fake-Actionserver und synthetische Daten, keine reale Fahrt.

**Rückfall:** Den einzelnen M3/M-Commit zurücknehmen oder
`wohnungserkundung_policy_enabled: false` lassen. Dann entfallen
Zielkandidatencache und Diagnose vollständig; Shadow, Legacy-, Action-, Nav2-
und Fahrpfade bleiben unverändert. Nichts wurde deployed oder an Hardware
aktiviert.
...
**Arbeitsbasis und Umfang:** `feature/we-m3k-frontier-goal-candidate` baut
direkt auf M3/J `1068262` auf. Das neue ROS-freie Modul nimmt genau eine
revisionsgebundene Absicht, die passende offene Frontier-Aufgabe, einen
vollständigen begrenzten Trackbestand, die M3/G-Skalarbelege und denselben exakt
korrelierten Rohkartensnapshot entgegen. Nur der eindeutige aktuelle Track darf
innerhalb der bereits verwendeten Clearance-/Seed-/Zielsuchgrenzen eine freie,
vom aktuellen Roboterseed geodätisch erreichbare Zielzelle liefern.

**Doppelte Belegprüfung und Ausgabegrenze:** Zielzelle, Route und
Informationsfläche werden erneut aus dem über Fingerprint, Stempel, Frame und
Revision gebundenen Zellbestand berechnet. Sie müssen den zuvor zur Auswahl
verwendeten Skalarbelegen entsprechen; dadurch kann weder eine fremde Karte
noch eine mit anderen Grenzwerten berechnete Bewertung still als Ziel dienen.
Die Ausgabe enthält nur Kontext-IDs, Quellbindung, Kartenzeile/-spalte sowie
endliche metrische X-/Y-/Yaw-, Weg- und Informationswerte. Sie ist weder
`PoseStamped` noch Actionziel oder Bewegungserlaubnis.

**Ausgeführte Prüfungen:** Lokaler x86_64-Arbeitsplatz mit ROS Humble und
frischem temporären Overlay; ausschließlich synthetische Raster/Positionen,
keine Geräte, realen Karten, Bags, Nodes, Actions oder Bewegung:

| Test-ID | Ergebnis |
|---|---|
| WE-M3K-FOCUS | Zielkandidatenadapter: **13 passed**; darunter exakter Kandidat, rotierter Ursprung, Frontierausrichtung, Kontext-/Revisions-/Aufgaben-/Trackfehler, fehlende Pose, blockierte Route, Skalarwiderspruch und harte Grenzen. |
| WE-M3K-EXPLORER | Vollständige Explorer-Suite: **709 passed**. |
| WE-M3K-ADJACENT | Explorer plus vollständige Fingerprint-, Karten-, Semantik-, Mission- und Launch-Suiten: **899 passed**. |
| WE-M3K-COLCON | Frischer Build von `amadeus_map_identity`, `robot_interfaces` und `explore`; **36 + 709 = 745 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3K-STATIC | `compileall`, auf geänderte Zeilen begrenztes `flake8` mit E501/W503-Ausnahmen und `git diff --check`: bestanden. |

**Offene Grenzen:** Der Zahlenkandidat ist nicht in den Node eingebunden und
wird weder in eine ROS-Pose umgewandelt noch an den vorhandenen Nav2-Client
übergeben. Nur Frontier-Aufgaben besitzen diesen metrischen Pfad; Portal- und
Beobachtungsaufgaben bleiben mangels spezifischer Zielevidenz gesperrt. Ein
Kindresultat wird noch nicht als Attempt/Retry verarbeitet. Abschlussruntime,
Profilgrenzen, Zielsystemlast und Hardwareprüfung fehlen. Softwaretests sind
keine Fahr- oder Hardwareabnahme.

**Nächster abgegrenzter Schritt WE-M3/L:** Einen reinen Ergebnisadapter vom
Kindzielautomaten zur bestehenden Aufgabenhistorie implementieren. Erfolg,
retrybarer Fehler, Invalidierung, Systemabbruch und Nutzer-Cancel müssen
unterschiedlich bleiben; insbesondere erledigt ein bloßer Nav-Erfolg die
Frontier-Aufgabe nicht. Keine Nav2-Übergabe oder Fahrwirkung.

**Rückfall:** Den einzelnen M3/K-Commit zurücknehmen. M3/J-Automat und alle
passiven Runtimepfade bleiben unverändert; das neue Modul besitzt keinen
Runtime-Aufrufer, Parameter, Publisher, Subscriber oder Actionpfad. Nichts wurde
deployed oder an Hardware aktiviert.

### 2026-09-14 – WE-M3/J: revisionsgebundener Einzel-Kindzielvertrag

**Arbeitsbasis und Umfang:** `feature/we-m3j-child-goal-contract` baut direkt
auf M3/I `1c4de65` auf. Das neue ROS-freie Modul leitet aus genau einer
zustandsbehafteten Auswahl eine deterministische, ausschließlich aus Kontext-,
Revisions-, Regions- und Aufgaben-ID bestehende Zielabsicht ab. Ein begrenzter
Sitzungsautomat besitzt höchstens eine aktive Absicht und führt Start,
Cancelanforderung und Kindresultat atomar. Der Schritt erzeugt bewusst weder
metrische Pose noch Pfad und ist noch nicht in den Explorer-Node eingebunden.

**Konflikt- und Invalidierungsregeln:** Exakte Replays sind nur zulässig,
solange Start beziehungsweise Cancel noch denselben aktiven Zustand bezeichnen;
terminale oder widersprüchlich wiederverwendete IDs werden abgewiesen. Ein
Resultatreplay liefert ausdrücklich ein als Duplikat markiertes identisches
Ergebnis. Fremder Kartenkontext, Resultate für ein anderes Kind und Revisionen
vor der Absicht scheitern geschlossen. Liegt eine invalidierende
Cancelanforderung vor, wird auch ein später gemeldeter Erfolg nicht als
Aufgabenerfolg übernommen, sondern als `invalidated` aufgelöst. Erst nach einem
terminalen Resultat darf die nächste Zielabsicht aktiv werden.
...
| Test-ID | Ergebnis |
|---|---|
| WE-M3J-FOCUS | Zielabsichts-/Kindzielvertrag: **13 passed**; darunter deterministische ID, konkurrierendes Kind, Start-/Cancel-/Resultatreplay, Revisionsinvalidierung, alle terminalen Ergebnisse, Fremdkontext und Kapazitätsgrenze. |
| WE-M3J-EXPLORER | Vollständige Explorer-Suite: **696 passed**. |
| WE-M3J-ADJACENT | Explorer plus vollständige Fingerprint-, Karten-, Semantik-, Mission- und Launch-Suiten: **886 passed**. |
| WE-M3J-COLCON | Frischer Build von `amadeus_map_identity`, `robot_interfaces` und `explore`; **36 + 696 = 732 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3J-STATIC | `compileall`, auf geänderte Zeilen begrenztes `flake8` mit E501/W503-Ausnahmen und `git diff --check`: bestanden. |

**Offene Grenzen:** Die Absicht enthält absichtlich keine metrische Pose. Es
existiert noch kein Zielkandidatenadapter, kein Nav2-Kindziel, kein
Runtime-Cancel und keine Rückführung eines Kindresultats in Attempt-/Retry- oder
Abschlusszustand. Portal-/Beobachtungsaufgaben besitzen weiter keine metrische
Zielevidenz. Profilgrenzen, Zielsystemlast und Hardwareprüfung fehlen.
Softwaretests sind keine Fahr- oder Hardwareabnahme.

**Nächster abgegrenzter Schritt WE-M3/K:** Einen reinen Adapter von exakt
korrelierter Rohkarte, ausgewählter Frontier-Aufgabe und ihrem stabilen Track zu
einem revisions- und framegebundenen metrischen Zielkandidaten implementieren.
Befahrbare Zielzelle, Ausrichtung, Eindeutigkeit und Suchgrenzen werden
synthetisch nachgewiesen; keine Nav2-Übergabe oder Fahrwirkung.

**Rückfall:** Den einzelnen M3/J-Commit zurücknehmen. M3/I-Auswahl und sämtliche
passiven Statuspfade bleiben unverändert; da das neue Modul keinen
Runtime-Aufrufer besitzt, gibt es keinen Parameter, Publisher, Subscriber,
Action- oder Fahrpfad zurückzuschalten. Nichts wurde deployed oder an Hardware
aktiviert.

### 2026-09-14 – WE-M3/I: zustandsbehaftete Auswahl in der Runtimediagnose

**Arbeitsbasis und Umfang:** `feature/we-m3i-stateful-runtime` baut direkt auf
M3/H `8c8b6b9` auf. Der vorhandene ROS-freie
`ExplorationTaskPolicySession` wird erst bei aktivierter WE-Policy und dem
ersten vollständigen Schattenkontext erzeugt. Der Node führt ihn höchstens
einmal je strikt neuer Kartenrevision fort und hält das Ergebnis getrennt von
der weiterhin bei jedem Statustakt neu berechneten passiven Frischebewertung.
Damit verändern Statusreplays weder Auswahlzähler noch Aufgabenalter oder
Regionshaltezeit.

**Sichere Auswahlprojektion:** Der reine Policykern wählt jetzt grundsätzlich
nichts, wenn seine vollständigen Quellen nicht frisch sind. Skalare Evidenz darf
den kompletten geeigneten Aufgabenbestand beschreiben; Retry-zurückgestellte oder
-erschöpfte IDs werden intern vor dem Scoring entfernt. Die Migration gibt
ausgewählte Aufgaben-/Regions-ID und Begründung nur als Diagnose aus. Wird die
aktuelle passive Bewertung innerhalb derselben Revision stale oder ist die
gespeicherte ID nicht mehr geeignet, werden beide IDs sofort auf `null` gesetzt
und der Grund lautet `withheld_by_current_passive_policy`. Es entsteht keine
Zielpose, kein Pfad und keine Ausführungserlaubnis.

Die Diagnose begrenzt Aufgabenbelege, Scores, Retrylisten und Historie jeweils
auf 128 Einträge; Gesamtzahlen und Abschneidung sind sichtbar. Pro Historieneintrag
werden ausschließlich ID, Region, erste/letzte Revision, Revisionsalter,
Auswahl-/Versuchszähler, Retrystand, letzter Grund, Reaktivierung und Erledigtstand
ausgegeben. Der alte Status bleibt bei deaktiviertem Opt-in unverändert.
...
| Test-ID | Ergebnis |
|---|---|
| WE-M3I-FOCUS | Policy-, Migration- und Nodevertrag: **143 passed**; darunter stale Auswahlblockade, exakte Retry-/Scorefilterung, begrenzte Projektion und einmalige Runtimefortschreibung je Revision. |
| WE-M3I-EXPLORER | Vollständige Explorer-Suite: **683 passed**. |
| WE-M3I-ADJACENT | Explorer plus vollständige Fingerprint-, Karten-, Semantik-, Mission- und Launch-Suiten: **873 passed**. |
| WE-M3I-COLCON | Frischer Build von `amadeus_map_identity`, `robot_interfaces` und `explore`; **36 + 683 = 719 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3I-ROS | Isolierte lokale DDS-Domain, direkter Explorer ohne Action: `ready_with_tasks`, Auswahl `task-frontier_000001`/`region_000001`, Grund `current_region`, Historie 1, Auswahlzähler trotz wiederholter Statusausgabe 1, `completion_allowed=false`, **0** Command-Nachrichten. |
| WE-M3I-STATIC | `compileall`, auf geänderte Zeilen begrenztes `flake8` mit E501/W503-Ausnahmen und `git diff --check`: bestanden. |

**Offene Grenzen:** Die Auswahl ist nur eine ID-Diagnose. Es existiert noch
keine revisionsgebundene Zielabsicht, keine Zielpose, kein Nav2-Kindziel, keine
Attempt-/Retryableitung aus einem Kindresultat und kein Cancel-/Wechselvertrag in
der Runtime. Fehlt Pose/Evidenz bei der ersten Bewertung einer Revision, bleibt
die zustandsbehaftete Auswahl konservativ bis zur nächsten Kartenrevision leer;
die stateless Diagnose zeigt die aktuelle Ursache weiter. Portal-/
Beobachtungsaufgaben besitzen noch keine metrische Zielevidenz. Abschlussruntime,
Profilgrenzen, Jetsonlast und Hardwareprüfung fehlen. Softwaretests sind keine
Fahr- oder Hardwareabnahme.

**Nächster abgegrenzter Schritt WE-M3/J:** Einen reinen Zielabsichts- und
Kindzielautomaten für genau eine revisionsgebundene ausgewählte Frontier-ID
implementieren. Zielwechsel, Cancel, Erfolg, retrybarer Fehler und
Revisionsinvalidierung müssen ohne konkurrierende aktive Kindziele und zunächst
nur über synthetische Ereignisse/Diagnose nachgewiesen werden. Noch keine
Nav2-Übergabe oder Fahrwirkung.

**Rückfall:** Den einzelnen M3/I-Commit zurücknehmen oder
`wohnungserkundung_policy_enabled: false` lassen. Dann existieren weder
zustandsbehaftete Runtimepolicy noch Auswahl-/Historiendiagnose; Schatten,
...
**Arbeitsbasis und Umfang:** `feature/we-m3g-frontier-evidence` baut direkt auf
M3/F `e0db4c3` auf. Das neue ROS-freie Modul
`frontier_task_evidence.py` nimmt ausschließlich den exakt korrelierten
Rohkartensnapshot, offene typisierte Graphaufgaben, unveränderliche stabile
Frontiertracks und optional die Roboterposition im Kartenframe entgegen. Der
Lebenszyklus gibt dazu nach Sitzungsstart eine unveränderliche Kopie der
Frontiertracks frei. Der Explorer berechnet die Evidenz erst außerhalb der
Schatten-Sperre und cachet sie nur für dieselbe Fingerprint-/Stempel-/Frame-/
Revisionsidentität und dieselbe Roboterrasterzelle.

**Evidenzvertrag:** Die Rohkarte wird mit der gemeinsamen kanonischen
Zellnormalisierung und Fingerprintfunktion erneut verifiziert. Pro offener
Aufgabe entsteht genau ein revisionsgebundener Zustand. Ein aktueller
Frontiertrack mit verfügbarer Pose, sicherer Startzelle, erreichbarer Zielzelle
und positiver unbekannter Fläche ist `available`. Fehlende Pose oder
Trackzuordnung bleibt `unknown`; fehlender sicherer Start oder Rohkartenweg ist
`temporarily_blocked`; fehlender Informationsgewinn ist `filtered`.
Portal-/Beobachtungsaufgaben bleiben mangels eigener Belegquelle ausdrücklich
`unknown`. Die Zielzelle wird zuerst geometrisch an die nächstgelegene sichere
Zelle gebunden und erst danach auf Erreichbarkeit geprüft, damit ein großer
Suchradius nicht durch eine Wand auf die roboterseitige Komponente springt.

Die Weglänge ist die skalare 8-Nachbar-Dijkstra-Distanz auf freien Zellen nach
euklidischer Hindernisabstandsprüfung; diagonales Eckenschneiden ist gesperrt.
Der Informationsgewinn ist die unbekannte Rasterfläche in einem metrischen
Radius um den stabilen Frontiermittelpunkt. Weder Rasterzelle, Pose, Pfad noch
Ziel werden ausgegeben. Die Statusmigration ergänzt begrenzt höchstens 128
Aufgabenbelege und 128 Scores; Anzahl und Abschneidung bleiben sichtbar, und
Scores müssen die geeigneten Aufgaben exakt abdecken.

**Vorläufige Grenzen und Messung:** Die Softwarestartwerte sind 0,28 m
Freiraum, 0,75 m Startzellensuche, 0,60 m Frontier-Zellensuche und 0,75 m
Informationsradius. Sie sind ausdrücklich nicht hardwarevermessen oder für eine
Abnahme eingefroren. Die Evidenzkarte ist hart auf 512 × 512 = 262.144 Zellen
begrenzt. Eine lokale x86_64-Messung verwarf die erste Python-Heap-Variante
(128²/256²/512²: 0,128/0,528/2,298 s; Spitzen-RSS bei 512² 59.948 KiB) zugunsten
des begrenzten SciPy-Sparse-Laufs (0,024/0,090/0,372 s; Spitzen-RSS bei 512²
112.884 KiB). Der höhere, aber begrenzte Speicherbedarf und die lokale Laufzeit
sind keine Jetsonmessung.
...
| Test-ID | Ergebnis |
|---|---|
| WE-M3G-FOCUS | Evidenz-, Lebenszyklus-, Migration- und Nodevertrag: **173 passed**; darunter Exaktidentität, Wand-/Ecktrennung, fehlende/stale Eingänge, Rasterrotation, vollständige Ordnung, Kapazität, Cache/Sperrdisziplin und importfreie Fahrgrenze. |
| WE-M3G-EXPLORER | Vollständige Explorer-Suite nach dem Wand-Suchradius-Randfall: **666 passed**. |
| WE-M3G-ADJACENT | Explorer plus vollständige Fingerprint-, Karten-, Semantik-, Mission- und Launch-Suiten: **856 passed**. |
| WE-M3G-COLCON | Frischer Build von `amadeus_map_identity`, `robot_interfaces` und `explore`; letzter Testlauf **36 + 666 = 702 Tests, 0 Fehler, 0 Fehlschläge, 0 Skips**. |
| WE-M3G-ROS | Isolierte lokale DDS-Domain, direkter Explorer ohne Action: ein 40 × 40-Snapshot ergab eine offene und geeignete Frontieraufgabe, 1,700000025 m Rohkartenweg, 0,900000027 m² Informationsfläche, `completion_allowed=false` und **0** Nachrichten auf beiden Explorer-Command-Topics. |
| WE-M3G-STATIC | `compileall`, auf geänderte Zeilen begrenztes `flake8` mit E501/W503-Ausnahmen und `git diff --check`: bestanden. |

Der erste synthetische Exaktjoin verwendete für die Fingerprintberechnung
Python-Double `0.1`, während die ROS-Nachricht die Auflösung als Float32
`0.10000000149…` transportiert. Der Join verweigerte dies korrekt. Mit dem
tatsächlich transportierten Float32-Wert bestand der Test. Ein weiterer
Prüfskriptlauf scheiterte ausschließlich an einem falsch auf oberster Ebene
statt unter `counts` gelesenen Diagnosefeld; das Produktfeld und die
abschließende korrigierte Prüfung waren korrekt.

**Offene Grenzen:** Trotz geeigneter Frontieraufgabe meldet die Policy im
ROS-Smoke korrekt `waiting_for_fresh_sources`: Der verbundene Portaldetektorfeed
führt einzelne Beobachtungen zu, aber keinen vollständigen Portalbestand. Bei
einem gültigen Leerbestand bleibt `portal_memory_revision` deshalb unbekannt.
Diese Lücke darf nicht durch erfundene Frische oder ein stillschweigendes
„keine Portale“ geschlossen werden. Weiter fehlen zustandsbehaftete Auswahl,
Versuchs-/Retryfortschreibung in der Runtime, Portal-/Beobachtungsevidenz,
Ziel-/Kindzielkorrelation, Abschlussbeobachtung, Zielsystemlast und jede
Hardwareprüfung. Softwaretests sind keine Fahr- oder Hardwareabnahme.

**Nächster abgegrenzter Schritt WE-M3/H:** Einen expliziten, korrelierten und
begrenzten vollständigen Portalinventarvertrag einschließlich Leerbestand durch
Adapter, Lebenszyklus und passive Runtime führen. Fehlende oder gescheiterte
Detektorauswertung bleibt veraltet. Rein diagnostisch muss der bestehende
Frontierfall danach bei gemeinsam frischen Quellen `ready_with_tasks` erreichen;
noch keine Auswahlfortschreibung, Zielbildung, Action, Nav2- oder Fahrwirkung.

**Rückfall:** `wohnungserkundung_policy_enabled: false` entfernt Bewertung und
Evidenzdiagnose; die anderen Schatten-Opt-ins bleiben getrennt. Vollständiger
Rückfall ist der einzelne M3/G-Revert. Legacy-Status, Action-, Navigations- und
Fahrpfade bleiben unverändert; nichts wurde deployed oder an Hardware aktiviert.
