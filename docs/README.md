...
Dieses Verzeichnis trennt aktuelle Betriebs- und Entwicklungsdokumentation von historischen Pruefprotokollen. Dadurch soll klar sein, welches Dokument eine heutige Entscheidung steuert und welches nur einen vergangenen Nachweis bewahrt.
...
`main` ist die kuenftige Integrationslinie. Neue Arbeit startet erst von `main`, nachdem die Mainline-PR gruen geprueft und gemerged ist. Kurzlebige Themenbranches beginnen von `main` und erhalten genau einen klaren Zweck.

## Aktive Dokumentation

| Bereich | Dokument | Zweck |
|---|---|---|
| Entscheidungen und Evidenz | [`PROJECT_MEMORY.md`](PROJECT_MEMORY.md) | Aktuelles fortlaufendes Projektgedaechtnis; neue, belegte Entscheidungen oben ergaenzen. |
...
| Arm/OAK | [`../KONZEPT_KALIBRIERUNG_OAK_ARM.md`](../KONZEPT_KALIBRIERUNG_OAK_ARM.md) | Hand-Auge-Kalibrierung; erst nach echter Arm-/Encoderabnahme ausfuehren. |

Die eigenstaendigen Plaene fuer Arm-Software sowie Diagnostik/Selbstbefreiung werden mit ihren jeweiligen PRs in diese Liste aufgenommen, sobald sie in der Mainline enthalten sind.

## Dokumentklassen

### Aktuell

Aktuelle Dokumentation beschreibt den vorgesehenen oder nachweislich laufenden Zustand. Jede Anpassung muss ein Datum, betroffene Komponenten, Teststatus und Rueckfallweg enthalten.
...
Ein neuer aktueller Pruefpfad wird erst nach der Mainline-Konsolidierung aus den bestehenden gezielten Pakettests, den Jetson-Abnahmen und den jeweils aktuellen Hardware-Checklisten abgeleitet. Er ersetzt die alte Sammelabnahme, nicht deren historische Nachweise.
